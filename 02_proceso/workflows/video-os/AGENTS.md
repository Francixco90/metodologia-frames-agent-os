...
## Invariantes (ver CONSTITUTION.md)

- **Estados no negociables**: `RENDERED_DRAFT != FINAL != HUMAN_APPROVED != READY != PUBLISHED`.
- **Evidence tags**: `[CÓDIGO]` `[CONFIG]` `[DOC]` `[INFERENCIA]` `[SUPUESTO]` o `coverage_gap` en toda decisión material.
- **Separación de roles**: producer, verifier y Guardian deben ser distintos. Lectura no concede escritura; una aprobación no autoriza efectos adyacentes.
...
- `feature_list.json` — Feature state tracker (source of truth; reemplaza `80_estado` duplicado)
