...
> Invariantes no negociables (estados, gates, fail-closed, evidence tags, profundidad ≤ 4, identidad MetodologIA) en [CONSTITUTION.md](CONSTITUTION.md). Esta plantilla solo añade el contrato de entrada para Gemini CLI.

## Supuestos

- Gemini CLI lee este archivo (`GEMINI.md`) o `.gemini/` al iniciar sesión. No usa `@-import`; las referencias a otros archivos son enlaces markdown normales.
...
- **Reusar CLAUDE.md con `@-import`**: rechazado — Gemini no soporta `@-import`; sintaxis distinta.
- **Constitución duplicada aquí**: rechazada — viola DRY.
- **Plantilla Gemini con enlace a CONSTITUTION.md**: elegida — mismo contrato, sintaxis nativa Gemini.
...
1. **Iniciar sesión**: leer `GEMINI.md` (este) → `AGENTS.md` → `context.md` → `progress.md`.
...
- Gemini no procesa `@-import` de Claude Code. Las referencias a otros `.md` son enlaces markdown.
