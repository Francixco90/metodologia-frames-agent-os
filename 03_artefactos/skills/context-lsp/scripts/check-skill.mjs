...
// Forbidden API calls must be absent across all governed files.
...
// Absolute path prefixes are constructed from char codes so the checker source
// does not contain the literal prefixes it scans for (no self-trip).
const fromCodes = (codes) => String.fromCharCode(...codes);
const prefixUsers = fromCodes([47, 85, 115, 101, 114, 115, 47]);
const prefixHome = fromCodes([47, 104, 111, 109, 101, 47]);
const prefixWinUsers = fromCodes([67, 58, 92, 85, 115, 101, 114, 115, 92]);
const absPathPattern = new RegExp(
  [prefixUsers, prefixHome, prefixWinUsers]
    .map((p) => p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
    .join('|'),
  'u',
);
if (absPathPattern.test(combined)) {
  failures.push('forbidden absolute path detected in governed files');
}
...
// LINEAGE.yml must declare 10 top-level fields.
