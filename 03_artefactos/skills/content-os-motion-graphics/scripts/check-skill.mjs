#!/usr/bin/env node
/**
 * content-os-motion-graphics local skill checker.
 *
 * Validates:
 ...
 *
 * Error prefix COSR-MG_.
 */
import {readFileSync, existsSync} from 'node:fs';
import {resolve} from 'node:path';

const ROOT = process.cwd();
const SKILL_DIR = resolve(ROOT, 'skills/content-os-motion-graphics');
const PREFIX = 'COSR-MG_';

const required = [
  'SKILL.md',
  'LINEAGE.yml',
  'schemas/motion-graphics-v1.schema.json',
...
if (errors.length > 0) {
  console.error(`FAIL content-os-motion-graphics: ${errors.length} error(s)`);
