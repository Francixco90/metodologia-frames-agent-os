#!/usr/bin/env node
/**
 * content-os-motion-graphics workflow audit.
 *
 * Validates a workflow-state (JSONL manifest branch) or brief (YAML branch)
 * against the motion-graphics workflow contract:
 *   - step order: setup, plan, source, design, build, verify, finalize
 *   - gate states: pending | in-progress | gate-passed | user-approved
 *   - unnarrated: vo_mode must be silent, has_script false (no narration)
 *   - no footage: footage must be false (no live-action)
 ...
 *   narration-in-motion-graphics, footage-in-motion-graphics
 *
...
const STEP_ORDER = ['setup', 'plan', 'source', 'design', 'build', 'verify', 'finalize'];
const VALID_STATES = new Set(['pending', 'in-progress', 'gate-passed', 'user-approved']);
const SCHEMA_VERSION = 'content-os-motion-graphics-audit-v1';
const VIOLATION_CODES = new Set([
  'missing-gate',
  'step-out-of-order',
  'no-render',
  'network-in-workflow',
  'narration-in-motion-graphics',
  'footage-in-motion-graphics',
]);
...
function auditObject(obj) {
  ...
  if (obj.route && obj.route !== 'content-os-motion-graphics') {
  ...
  // narration checks
  if (obj.vo_mode !== undefined && obj.vo_mode !== 'silent') {
    violations.push({code: 'narration-in-motion-graphics', detail: `vo_mode ${obj.vo_mode}`});
  }
  if (obj.has_script === true) {
    violations.push({code: 'narration-in-motion-graphics', detail: 'has_script true'});
  }
  // footage checks
  if (obj.footage === true) {
    violations.push({code: 'footage-in-motion-graphics', detail: 'footage true'});
  }
