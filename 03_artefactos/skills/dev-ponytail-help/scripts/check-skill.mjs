...
const id = 'dev-ponytail-help';
const required = [
  `skills/${id}/SKILL.md`,
  `skills/${id}/LINEAGE.yml`,
  `skills/${id}/scripts/check-skill.mjs`,
  `skills/${id}/receipts/runtime-boundary.yml`,
  `skills/${id}/fixtures/positive/show-reference-card.yml`,
  `skills/${id}/fixtures/negative/auto-invoke-command-from-help.yml`,
];
...
for (const token of [id, 'referencia', 'card', 'modos']) {
  if (!combined.includes(token)) {
...
const negative = contents.get(`skills/${id}/fixtures/negative/auto-invoke-command-from-help.yml`);
