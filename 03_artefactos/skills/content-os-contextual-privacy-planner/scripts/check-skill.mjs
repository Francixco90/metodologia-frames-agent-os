...
const buildInventory = (signals = cases.signals, candidateCoverage = coverage, status = 'BLOCKED_PENDING_PRIVACY_POLICY') => seal({schema_version: 'sensitive-signal-inventory-v1', inventory_id: 'INV-PRIVACY-PLANNER', case_id: cases.case_id, source: cases.source, detector_actor_id: 'RT-07-H03-PRIVACY-DETECTOR-PRODUCER', aliases_sha256: digest(cases.aliases), templates_sha256: '2'.repeat(64), coverage: candidateCoverage, signals, status});
...
const unknownSilentRequest = buildRequest({inventory: buildInventory(silentSignals, {...coverage, audio_transcript: 'UNKNOWN'}, 'BLOCKED_SIGNAL_CONFIDENCE_UNKNOWN'), decisions: cases.decisions.filter(({signal_id}) => signal_id !== 'SIG-GNP-AUDIO'), metadata: {...cases.source_metadata, has_audio: false}});
if (planContextualPrivacy(unknownSilentRequest).redaction_plan.status !== 'BLOCKED_PENDING_HUMAN_PRIVACY_REVIEW') throw new Error('CPP-NO-AUDIO-UNKNOWN-POSITIVE');
...
candidate = clone(); delete candidate.aliases_material; expectBlocked('aliases-material-missing', candidate, 'PLANNER-REQUEST-KEYS');
...
candidate = clone(); for (const field of ['aliases_material', 'inventory_material', 'inventory_verification_receipt', 'source_probe_receipt', 'directive_material']) candidate[field].ref = 'evidence/shared.json'; expectBlocked('material-ref-reuse', candidate, 'PLANNER-MATERIAL-REF-ALIAS');
candidate = clone(); rewrite(candidate, 'aliases_material', (value) => { value[0].variants.push('Naty'); }); expectBlocked('aliases-material-drift', candidate, 'PLANNER-ALIASES-DRIFT');
candidate = clone(); rewrite(candidate, 'aliases_material', (value) => { value[1].alias_id = value[0].alias_id; }); const duplicateIdAliases = parse(candidate.aliases_material); reissueInventory(candidate, (inventory) => { inventory.aliases_sha256 = digest(duplicateIdAliases); }); expectBlocked('aliases-duplicate-id', candidate, 'PLANNER-ALIAS-INVALID');
candidate = clone(); rewrite(candidate, 'aliases_material', (value) => { value.push(...Array.from({length: 511}, (_, index) => ({alias_id: `ALIAS-LIMIT-${index}`, kind: 'NAME', canonical: `Identity${index}`, variants: [`Identity${index}`]}))); }); const excessiveAliases = parse(candidate.aliases_material); reissueInventory(candidate, (inventory) => { inventory.aliases_sha256 = digest(excessiveAliases); }); expectBlocked('aliases-limit', candidate, 'PLANNER-ALIASES-DRIFT');
...
candidate = clone(); reissueInventory(candidate, (inventory) => { inventory.signals[0].identity.matched_alias = 'Sofka'; }); expectBlocked('inventory-cross-alias', candidate, 'PLANNER-SIGNAL-ALIAS-BINDING');
candidate = clone(); rewrite(candidate, 'aliases_material', (value) => { value[0].variants.push('Naty'); }); const variantAliases = parse(candidate.aliases_material); reissueInventory(candidate, (inventory) => { inventory.aliases_sha256 = digest(variantAliases); inventory.signals[0].identity.matched_alias = 'Naty'; }); if (planContextualPrivacy(candidate).privacy_policy.rules[0].identity !== 'Natalia') throw new Error('CPP-EXPLICIT-ALIAS-POSITIVE');
candidate = clone(); rewrite(candidate, 'aliases_material', (value) => { value[0].variants.push('Naty'); }); const duplicateAliases = parse(candidate.aliases_material); reissueInventory(candidate, (inventory) => { inventory.aliases_sha256 = digest(duplicateAliases); const duplicate = structuredClone(inventory.signals[0]); duplicate.signal_id = 'SIG-NATALIA-DUP'; duplicate.sequence = inventory.signals.length; duplicate.identity.matched_alias = 'Naty'; duplicate.evidence = {observation_id: 'OBS-NATALIA-DUP', material: {ref: 'evidence/natalia-duplicate.json', sha256: '9'.repeat(64), bytes: 21}}; inventory.signals.push(duplicate); }); rewrite(candidate, 'directive_material', (value) => { value.decisions.push({signal_id: 'SIG-NATALIA-DUP', rule: 'PROTECT', reason_code: 'SENSITIVE_IDENTITY'}); }); expectBlocked('inventory-semantic-duplicate-alias-variant', candidate, 'PLANNER-INVENTORY-SIGNAL-DUPLICATE');
...
for (const [kind, identity] of [['FILE_PATH', '2026/08/21'], ['FILE_PATH', 'folder/subfolder'], ['EMAIL', 'student@example.c'], ['URL', 'https://example.test/demo)]']]) { candidate = clone(); reissueInventory(candidate, (inventory) => { inventory.signals[3].kind = kind; inventory.signals[3].identity.canonical = identity; }); expectBlocked('inventory-structured-detector-parity', candidate, 'PLANNER-SIGNAL-STRUCTURED-IDENTITY'); }
...
console.info(`PASS contextual-privacy-planner: ${bundle.privacy_policy.rules.length} rules, ${bundle.value_preservation_plan.zones.length} value zone and 72 adversarial gates.`);
