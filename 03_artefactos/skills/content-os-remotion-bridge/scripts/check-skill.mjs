#!/usr/bin/env node
/**
 * content-os-remotion-bridge local skill checker.
 *
 * Validates:
 ...
 *
 * Error prefix COSR-RB_.
 */
import {readFileSync, existsSync} from 'node:fs';
import {resolve} from 'node:path';

const ROOT = process.cwd();
const SKILL_DIR = resolve(ROOT, 'skills/content-os-remotion-bridge');
const PREFIX = 'COSR-RB_';

const required = [
  'SKILL.md',
  'LINEAGE.yml',
  'schemas/remotion-bridge-v1.schema.json',
  'scripts/check-skill.mjs',
  'scripts/workflow-audit.mjs',
  'references/api-map.md',
  'references/inverse-map.md',
  'references/escape-hatch.md',
  'rules/workflow-contract.md',
  'examples/translation-brief.jsonl',
  'examples/step-sequence.jsonl',
...
if (errors.length > 0) {
  console.error(`FAIL content-os-remotion-bridge: ${errors.length} error(s)`);
  for (const e of errors) console.error(`  ${e}`);
  process.exitCode = 1;
} else {
  console.info(
    'PASS content-os-remotion-bridge: 14 files, frontmatter, tokens, forbidden-scan, negative-completeness.',
  );
}
