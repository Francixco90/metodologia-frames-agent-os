 ...
 * (schemaVersion content-os-faceless-explainer-audit-v1). --strict exits
 * nonzero. Static (no deps): deterministic, offline, fast.
 *
 * Usage: node skills/content-os-faceless-explainer/scripts/workflow-audit.mjs <workflow-state> --out <dir> [--strict]
 */
...
const collectEntry = (entry, id) => {
  if (!entry.route || entry.route !== 'content-os-faceless-explainer') {
  ...
  if (entry.capture === true) {
    violations.push({
      code: 'capture-in-faceless',
      detail: `project ${id} capture=true in faceless workflow`,
    });
  }
  if (entry.footage === true) {
    violations.push({
      code: 'footage-in-faceless',
      detail: `project ${id} footage=true in faceless workflow`,
    });
  }
};
...
if (isManifest) {
  ...
  for (const rawLines of blocks) {
    ...
    const hasCapture = /\bcapture:\s*true\b/u.test(raw);
    const hasFootage = /\bfootage:\s*true\b/u.test(raw);
    const offlineTrue = /\boffline:\s*true\b/u.test(raw);
    const stepCount = (raw.match(/^\s*-\s+step:\s*/gmu) || []).length;
    ...
    if (!route || route !== 'content-os-faceless-explainer') {
      violations.push({code: 'missing-gate', detail: `project ${id} missing or invalid route`});
    }
...
const audit = {
  schemaVersion: 'content-os-faceless-explainer-audit-v1',
  ...
  faceless: !violations.some(
    (v) => v.code === 'capture-in-faceless' || v.code === 'footage-in-faceless',
  ),
...
console.info(
  `PASS workflow-audit: gateBound=${audit.gateBound}, ordered=${audit.ordered}, faceless=${audit.faceless}, offlineFirst=${audit.offlineFirst}, violations=${violations.length} -> ${outPath}`,
);
