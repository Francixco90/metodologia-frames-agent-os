...
const retainedCrop = (operation, metadata) => ({TOP: {x: 0, y: operation.authorized_effect_roi.height, width: metadata.frame_width, height: metadata.frame_height - operation.authorized_effect_roi.height}, BOTTOM: {x: 0, y: 0, width: metadata.frame_width, height: operation.authorized_effect_roi.y}, LEFT: {x: operation.authorized_effect_roi.width, y: 0, width: metadata.frame_width - operation.authorized_effect_roi.width, height: metadata.frame_height}, RIGHT: {x: 0, y: 0, width: operation.authorized_effect_roi.x, height: metadata.frame_height}}[operation.reframe_edge]);
const mappedRoi = (roi, crop, metadata, expand) => { const sx = expand ? Math.ceil(6 * metadata.frame_width / crop.width) : 0; const sy = expand ? Math.ceil(6 * metadata.frame_height / crop.height) : 0; const left = Math.max(0, Math.floor((roi.x - crop.x) * metadata.frame_width / crop.width) - sx); const top = Math.max(0, Math.floor((roi.y - crop.y) * metadata.frame_height / crop.height) - sy); const right = Math.min(metadata.frame_width, Math.ceil((roi.x + roi.width - crop.x) * metadata.frame_width / crop.width) + sx); const bottom = Math.min(metadata.frame_height, Math.ceil((roi.y + roi.height - crop.y) * metadata.frame_height / crop.height) + sy); return right > left && bottom > top ? {x: left, y: top, width: right - left, height: bottom - top} : null; };
const subtractRoi = (roi, protectedRoi) => { const left = Math.max(roi.x, protectedRoi.x); const top = Math.max(roi.y, protectedRoi.y); const right = Math.min(roi.x + roi.width, protectedRoi.x + protectedRoi.width); const bottom = Math.min(roi.y + roi.height, protectedRoi.y + protectedRoi.height); if (right <= left || bottom <= top) return [roi]; return [{x: roi.x, y: roi.y, width: roi.width, height: top - roi.y}, {x: roi.x, y: bottom, width: roi.width, height: roi.y + roi.height - bottom}, {x: roi.x, y: top, width: left - roi.x, height: bottom - top}, {x: right, y: top, width: roi.x + roi.width - right, height: bottom - top}].filter(({width, height}) => width > 0 && height > 0); };
const outputMasks = (operations, zones, metadata) => { const reframes = operations.filter(({type}) => type === 'REFRAME_PERIPHERAL'); const masks = []; for (const operation of operations.filter(({type}) => type === 'LOCAL_BLUR')) { const protectedZones = zones.filter((zone) => !zone.authorized_redaction_signal_ids.includes(operation.signal_id) && overlaps(operation.frame_span, zone.frame_span)); const boundaries = new Set([operation.frame_span.start, operation.frame_span.end + 1]); for (const item of [...reframes, ...protectedZones]) if (overlaps(operation.frame_span, item.frame_span)) { boundaries.add(Math.max(operation.frame_span.start, item.frame_span.start)); boundaries.add(Math.min(operation.frame_span.end, item.frame_span.end) + 1); } const ordered = [...boundaries].sort((a, b) => a - b); for (let index = 0; index < ordered.length - 1; index += 1) { const start = ordered[index]; const end = ordered[index + 1] - 1; const active = reframes.find(({frame_span}) => frame_span.start <= start && frame_span.end >= end); const crop = active ? retainedCrop(active, metadata) : null; const roi = crop ? mappedRoi(operation.authorized_effect_roi, crop, metadata, true) : operation.authorized_effect_roi; let pieces = roi ? [roi] : []; for (const zone of protectedZones.filter(({frame_span}) => frame_span.start <= start && frame_span.end >= end)) { const focal = crop ? mappedRoi(zone.geometry, crop, metadata, false) : zone.geometry; if (focal) pieces = pieces.flatMap((piece) => subtractRoi(piece, focal)); } for (const piece of pieces) masks.push({frame_span: {start, end}, roi: piece}); } } return masks; };
const frameDigests = (path, masks, ffmpeg) => { const boxes = masks.map(({frame_span, roi}) => `drawbox=x=${roi.x}:y=${roi.y}:w=${roi.width}:h=${roi.height}:color=black:t=fill:enable='between(n\\,${frame_span.start}\\,${frame_span.end})'`); return command(ffmpeg, ['-v', 'error', '-i', path, '-map', '0:v:0', '-vf', [...boxes, 'format=rgb24'].join(','), '-f', 'framemd5', '-']).split('\n').filter((line) => line && !line.startsWith('#')).map((line) => line.split(',').map((item) => item.trim()).at(-1)); };
const renderExpectedReframes = (sourcePath, destination, operations, metadata, ffmpeg) => { const filters = ['[0:v:0]setpts=PTS-STARTPTS[v0]']; let video = 'v0'; operations.forEach((operation, sequence) => { const index = sequence + 1; const current = video; video = `v${index}`; const crop = retainedCrop(operation, metadata); filters.push(`[${current}]split=2[base${index}][src${index}]`, `[src${index}]format=yuv444p,crop=${crop.width}:${crop.height}:${crop.x}:${crop.y}:exact=1,scale=${metadata.frame_width}:${metadata.frame_height}:flags=lanczos[scaled${index}]`, `[base${index}][scaled${index}]overlay=0:0:enable='between(n\\,${operation.frame_span.start}\\,${operation.frame_span.end})'[${video}]`); }); command(ffmpeg, ['-v', 'error', '-i', sourcePath, '-filter_complex', filters.join(';'), '-map', `[${video}]`, '-an', '-frames:v', String(metadata.frame_count), '-map_metadata', '-1', '-c:v', 'libx264', '-preset', 'medium', '-crf', '0', '-pix_fmt', 'yuv420p', destination]); };
const rawFrameManifest = (path, ffmpeg) => frameDigests(path, [], ffmpeg);
export const visualMismatches = (sourcePath, outputPath, operations, zones, metadata, ffmpeg) => { const masks = outputMasks(operations, zones, metadata); const reframes = operations.filter(({type}) => type === 'REFRAME_PERIPHERAL'); const scratch = reframes.length ? mkdtempSync(resolve(tmpdir(), 'publication-reframe-')) : null; const expectedPath = scratch ? resolve(scratch, 'expected.mp4') : sourcePath; try { if (scratch) renderExpectedReframes(sourcePath, expectedPath, reframes, metadata, ffmpeg); const expected = frameDigests(expectedPath, masks, ffmpeg); const actual = frameDigests(outputPath, masks, ffmpeg); expect(expected.length === metadata.frame_count && actual.length === metadata.frame_count, 'PUBLICATION-FRAME-COUNT'); return expected.flatMap((hash, index) => hash === actual[index] ? [] : [index]); } finally { if (scratch) rmSync(scratch, {recursive: true, force: true}); } };
...
const videoShape = (path, ffprobe) => { const raw = JSON.parse(command(ffprobe, ['-v', 'error', '-count_frames', '-select_streams', 'v:0', '-show_entries', 'stream=r_frame_rate,avg_frame_rate,nb_read_frames:format=duration', '-of', 'json', path])); const video = raw.streams?.[0]; expect(video && video.r_frame_rate === video.avg_frame_rate && /^\d+\/\d+$/u.test(video.r_frame_rate), 'PUBLICATION-VIDEO-SHAPE'); return {fps: video.r_frame_rate, frame_count: Number(video.nb_read_frames), duration_ms: Math.round(Number(raw.format?.duration) * 1000)}; };
...
const deriveReport = (request, options) => {
  keys(request, ['schema_version', 'case_id', 'participant_id', 'actor_id', 'verification_as_of', 'execution_request_material', 'execution_receipt_material', 'pre_detection', 'post_detection', 'post_rescan_receipt', 'class_memory_authorization_material', 'authorization_freshness_receipt', 'disclosure_curtains', 'expected_export_set_material', 'disclosure_export_manifest_material', 'export_set_verification_receipt'], 'PUBLICATION-REQUEST-KEYS');
  ...
  const outputVideo = videoShape(outputPath, ffprobe); const outputAudio = audioShape(outputPath, ffprobe); const rawFrameManifestSha = digest(rawFrameManifest(outputPath, ffmpeg)); const rawAudioPcmSha = outputAudio ? sha(pcm(outputPath, ffmpeg, outputAudio)) : null;
  const rescan = physical(request.post_rescan_receipt, 'PUBLICATION-POST-RESCAN'); keys(rescan, ['schema_version', 'actor_id', 'case_id', 'source_sha256', 'verification_as_of', 'aliases_sha256', 'templates_sha256', 'detector_request_material_sha256', 'inventory_material_sha256', 'coverage_sha256', 'ffmpeg_sha256', 'ffprobe_sha256', 'video_rgb_manifest_sha256', 'audio_pcm_sha256', 'observed_frame_count', 'observed_duration_ms', 'status'], 'PUBLICATION-POST-RESCAN-KEYS'); expect(rescan.schema_version === 'publication-sensitive-rescan-execution-v1' && rescan.actor_id === 'RT-09-H03-PUBLICATION-RESCAN-VERIFIER' && rescan.case_id === request.case_id && rescan.source_sha256 === executionReceipt.output.sha256 && rescan.verification_as_of === request.verification_as_of && rescan.aliases_sha256 === pre.detectorRequest.aliases_sha256 && rescan.templates_sha256 === pre.detectorRequest.templates_sha256 && rescan.detector_request_material_sha256 === request.post_detection.request_material.sha256 && rescan.inventory_material_sha256 === request.post_detection.inventory_material.sha256 && rescan.coverage_sha256 === digest(post.detectorRequest.coverage) && rescan.ffmpeg_sha256 === executionRequest.media_tool_authority.ffmpeg_sha256 && rescan.ffprobe_sha256 === executionRequest.media_tool_authority.ffprobe_sha256 && rescan.video_rgb_manifest_sha256 === rawFrameManifestSha && rescan.audio_pcm_sha256 === rawAudioPcmSha && rescan.observed_frame_count === outputVideo.frame_count && rescan.observed_duration_ms === outputVideo.duration_ms && rescan.status === 'VERIFIED_COMPLETE_PUBLICATION_RESCAN', 'PUBLICATION-POST-RESCAN-DRIFT');

  const {protectedRules, residuals} = protectedResiduals(policy.rules, post.inventory);
  const rgbMismatches = visualMismatches(sourcePath, outputPath, plan.operations, valuePlan.zones, metadata, ffmpeg);
  const events = new Map();
  for (const operation of plan.operations.filter(({type}) => ['LOCAL_BLUR', 'REFRAME_PERIPHERAL'].includes(type))) { const ratio = operation.authorized_effect_roi.width * operation.authorized_effect_roi.height / (metadata.frame_width * metadata.frame_height); events.set(operation.frame_span.start, (events.get(operation.frame_span.start) ?? 0) + ratio); events.set(operation.frame_span.end + 1, (events.get(operation.frame_span.end + 1) ?? 0) - ratio); }
  ...
  const occlusions = [];
  for (const operation of plan.operations.filter(({type}) => ['LOCAL_BLUR', 'REFRAME_PERIPHERAL'].includes(type))) for (const zone of valuePlan.zones) if (overlaps(operation.frame_span, zone.frame_span) && intersects(operation.authorized_effect_roi, zone.geometry) && (operation.type === 'REFRAME_PERIPHERAL' || !zone.authorized_redaction_signal_ids.includes(operation.signal_id))) occlusions.push({sequence: occlusions.length, zone_id: zone.zone_id, signal_id: operation.signal_id, operation_id: operation.operation_id});

  const shape = audioShape(sourcePath, ffprobe); expect(same(shape, outputAudio) && Boolean(shape) === metadata.has_audio, 'PUBLICATION-AUDIO-SHAPE'); const video = outputVideo; expect(video.frame_count === metadata.frame_count && video.duration_ms === metadata.duration_ms, 'PUBLICATION-VIDEO-METADATA');
