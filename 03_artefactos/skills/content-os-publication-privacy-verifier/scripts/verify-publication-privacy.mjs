...
const deriveReport = (request, options) => {
  ...
  const outputVideo = videoShape(outputPath, ffprobe); const outputAudio = audioShape(outputPath, ffprobe); const rawFrameManifestSha = digest(frameDigests(outputPath, [], ffmpeg)); const rawAudioPcmSha = outputAudio ? sha(pcm(outputPath, ffmpeg, outputAudio)) : null;
