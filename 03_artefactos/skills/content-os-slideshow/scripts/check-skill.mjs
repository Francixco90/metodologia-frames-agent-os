#!/usr/bin/env node
/**
 * content-os-slideshow local skill checker.
 *
 * Validates:
 ...
 *
 * Error prefix COSR-SL_.
 */
import {readFileSync, existsSync} from 'node:fs';
import {resolve} from 'node:path';

const ROOT = process.cwd();
const SKILL_DIR = resolve(ROOT, 'skills/content-os-slideshow');
const PREFIX = 'COSR-SL_';

const required = [
  'SKILL.md',
  'LINEAGE.yml',
  'schemas/slideshow-v1.schema.json',
  'scripts/check-skill.mjs',
  'scripts/workflow-audit.mjs',
  'references/island-schema.md',
...
if (errors.length > 0) {
  console.error(`FAIL content-os-slideshow: ${errors.length} error(s)`);
  for (const e of errors) console.error(`  ${e}`);
  process.exitCode = 1;
} else {
  console.info(
    'PASS content-os-slideshow: 14 files, frontmatter, tokens, forbidden-scan, negative-completeness.',
  );
}
