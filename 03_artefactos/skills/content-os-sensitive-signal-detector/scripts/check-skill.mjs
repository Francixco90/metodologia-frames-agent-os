...
const bindObservation = (observation) => {
  ...
  const evidence = canonicalMaterial(`evidence/${core.observation_id.toLowerCase()}.json`, {schema_version: 'sensitive-signal-observation-evidence-v1', actor_id: actors[core.modality], case_id: caseId, source_sha256: source.sha256, observation_id: core.observation_id, modality: core.modality, observation_sha256: digest(Object.fromEntries(Object.entries(core).filter(([key]) => key !== 'observation_id')))});
  return {...core, evidence};
};
const bindCoverage = (request) => {
  request.coverage_receipt = canonicalMaterial('evidence/coverage.json', {schema_version: 'sensitive-signal-coverage-receipt-v1', actor_id: 'RT-09-PRIVACY-COVERAGE-VERIFIER', case_id: request.case_id, source_sha256: request.source.sha256, coverage: request.coverage});
};
const observations = cases.observations.map(({evidence: _evidence, ...item}) => bindObservation(item));
const request = {
  schema_version: 'sensitive-signal-detector-request-v1', case_id: caseId, source,
...
expectBlocked('source-evidence-reuse', (value) => { const replacement = Buffer.from('other-source-analysis'); value.source = {...value.source, ...material('sources/other-class.bin', replacement)}; bindCoverage(value); }, 'DETECTOR-EVIDENCE-DRIFT');
expectBlocked('case-evidence-reuse', (value) => { value.case_id = 'CASE-OTHER'; bindCoverage(value); }, 'DETECTOR-EVIDENCE-DRIFT');
...
console.info(`PASS sensitive-signal-detector: ${inventory.signals.length} signals and 36 adversarial gates.`);
