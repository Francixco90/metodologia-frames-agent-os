...
const id = 'dev-karpathy-guidelines';
const required = [
  `skills/${id}/SKILL.md`,
  `skills/${id}/LINEAGE.yml`,
  `skills/${id}/scripts/check-skill.mjs`,
  `skills/${id}/receipts/runtime-boundary.yml`,
  `skills/${id}/fixtures/positive/apply-surgical-changes.yml`,
  `skills/${id}/fixtures/negative/overcomplicate-simple-fix.yml`,
];
...
for (const token of ['dev-karpathy-guidelines', 'quirúrgico', 'supuestos', 'éxito']) {
  if (!combined.includes(token)) {
    throw new Error(`${id.toUpperCase()}_CONTRACT_MISSING: ${token}`);
  }
}
...
const negative = contents.get(`skills/${id}/fixtures/negative/overcomplicate-simple-fix.yml`);
