...
- **Stage ground:** `#root` must be opaque
  ...
  summed-opacity < 1 window that flashes white otherwise (see `content-os-seam-craft`).
...
- `content-os-motion-doctrine` — the gateway (vector law, the current, the Seam Gate).
- `content-os-seam-craft` — render prerequisites (opaque stage ground / white-flash guard).
...
- Does not author seams without the vector ledger (read content-os-motion-doctrine first).
- Does not allow full off-screen moves (partial travel ~12% + early fade).
- Does not permit `.inOut` eases on either side of a cut (mirrored power4.in / power4.out).
- Does not blur children (blur the wrapper) or blur `<video>` directly.
...
`node skills/content-os-cut-the-curve/scripts/check-skill.mjs` — verifies required
