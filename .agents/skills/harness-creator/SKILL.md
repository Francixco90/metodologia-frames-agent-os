...
description: >-
  Build, audit, and improve self-orchestrated harnesses that make AI coding agents reliable:
  AGENTS.md/CLAUDE.md instruction files, feature/state tracking, verification gates, scope
  boundaries, session handoff, memory persistence, context budgets, tool-permission safety, and
  multi-agent coordination. Use this whenever a coding agent is unreliable across sessions —
  forgets context, drifts out of scope, claims "done" before tests pass, or starts each session
  inconsistently — or when creating or assessing AGENTS.md, CLAUDE.md, feature_list.json, init.sh,
  progress.md, or session-handoff files. Reach for it even if the user never says the word "harness."
license: MIT
---
...
| Orchestration | 5 .md files + gates + workflow schema | Self-orchestration declaration and DAG |
| Structure | `00_inbox`/`99_archive` + depth limit | Indexable, navigable layout |

## Self-Orchestration Principle

Este sistema convierte intención en resultados por procesos auto orquestado. [DOC]

Every generated harness declares this preamble in its 5 orchestration .md files (instructions, state, verification, scope, lifecycle), making the DAG self-describing and agent-driven rather than externally scripted. [CONFIG]

## Folder Taxonomy

Harness folders follow a numeric-prefix convention so any agent can navigate the layout without a map. [CONFIG]

| Folder | Purpose |
|---|---|
| `00_inbox` | Raw inputs pending triage |
| `01_intencion` | Stated intent and goal capture |
| `10_proceso/gates` | Verification gates |
| `10_proceso/workflows` | Workflow definitions |
| `20_artefactos` | Produced artifacts |
| `80_estado` | State and progress |
| `90_verificacion` | Verification evidence |
| `99_archive` | Completed and archived work |

Folder name regex: `^\d{2}_[a-z][a-z0-9-]*$`. Maximum depth: 4. [CONFIG]

## Seven Subsystems

The five core subsystems above plus Orchestration and Structure form the seven subsystems scored by `validate-harness.mjs`: [DOC]
