...
export const SUBSYSTEMS = ['instructions', 'state', 'verification', 'scope', 'lifecycle', 'orchestration', 'structure'];

const IGNORED_DIRS = new Set(['.git', 'node_modules', 'dist', 'build', '.next', '.venv', 'venv', '__pycache__']);

// Recursive directory depth. Root counts as 0; each nested dir adds 1.
// Reuses the same ignored set as listFiles so virtualenvs/build artifacts don't inflate depth.
  ...
  function walk(current, depth) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        const childDepth = walk(path.join(current, entry.name), depth + 1);
...
// Checks whether top-level directories follow the ^\d{2}_[a-z] naming convention.
function dirsFollowConvention(root) {
  let entries = [];
  try {
  ...
  const dirs = entries.filter((e) => e.isDirectory() && !IGNORED_DIRS.has(e.name)).map((e) => e.name);
  if (dirs.length === 0) return false;
  return dirs.some((d) => /^\d{2}_[a-z]/.test(d));
}
...
export function scoreHarness(files, root = null) {
  ...
  // Orchestration: the preamble must appear verbatim in each of the 5 .md files.
  const preamble = 'convierte intención en resultados por procesos auto orquestado';
  const preambleFiles = ['CLAUDE.md', 'GEMINI.md', 'AGENTS.md', 'README.md', 'CONSTITUTION.md'];
  const preambleInAll = preambleFiles.every((name) =>
    (byPath.get(name) || '').toLowerCase().includes(preamble));
  const gatesAllPresent = ['APROBAR', 'REVISAR', 'CANCELAR'].every((g) =>
    allText.toLowerCase().includes(g.toLowerCase()));
  const statesAllPresent = ['RECIBIDO', 'ENTREGADO'].every((s) =>
  ...
  const checks = {
    ...
    orchestration: [
      { pass: preambleInAll, message: 'Preamble verbatim present in all 5 .md files' },
      { pass: gatesAllPresent, message: 'Gates APROBAR/REVISAR/CANCELAR mentioned' },
      { pass: statesAllPresent, message: 'States RECIBIDO/ENTREGADO mentioned' },
      textHas(allText, ['producer', 'contexto fresco', 'independiente'], 'Independent verifier referenced'),
      textHas(allText, ['workflow.schema.json', 'workflow'], 'Workflow schema referenced')
    ],
    structure: [
      textHas(allText, ['00_inbox'], '00_inbox mentioned or present'),
      textHas(allText, ['99_archive'], '99_archive mentioned or present'),
      root ? { pass: maxDepth(root) <= 4, message: 'Directory depth ≤ 4' } : { pass: true, message: 'Directory depth ≤ 4 (skipped: no root)' },
      root ? { pass: dirsFollowConvention(root), message: 'Folder names match ^\\d{2}_[a-z] convention' } : { pass: true, message: 'Folder names match convention (skipped: no root)' },
      textHas(allText, ['symlink', 'retro'], 'Symlinks retro mentioned or present')
    ]
  };
...
export async function loadHarnessFiles(root) {
  const candidates = [
    ...
    'session-handoff.md',
    'init.sh',
    '80_estado/feature_list.json',
    '80_estado/progress.md',
    '80_estado/session-handoff.md',
    '90_verificacion/init.sh'
  ];
  const files = [];
...
<body>
  <main>
    <header>
      ...
      <p>Seven-subsystem harness validation report.</p>
      <div class="summary">
