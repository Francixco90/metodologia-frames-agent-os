...
const addMatchingFiles = (
  ...
  for (const relativePath of discovered) {
    // Collect every matching file. Binary assets are rejected below by the
    // NUL-byte guard; extension allowlists would silently miss executable
    // renderer formats such as .mjs/.cjs/.jsx/.mts. [CONFIG]
...
export const scanLocatorsAndColors = (root: string): string[] => {
  ...
  // Presentation surfaces may embed governed token projections, so BR004 keeps
  // its authored-source allowlist. BR009 still covers every renderer, workflow
  // template and materialized HTML surface. [CONFIG]
