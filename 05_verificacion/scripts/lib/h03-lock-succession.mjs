...
export const H03_LOCK_SUCCESSION_REF = 'receipts/dependency-audits/H03-LOCK-SUCCESSION-002.yml';
...
export const verifyApprovedH03LockSuccession = (
  root,
  {lockfileRef = 'pnpm-lock.yaml', previousLockSha256 = H02_LOCK_SHA256} = {},
) => {
  ...
  if (
    ...
    receipt.receipt_id !== 'H03-LOCK-SUCCESSION-002' ||
    receipt.supersedes_receipt_id !== 'H03-LOCK-SUCCESSION-001' ||
    receipt.approval_phrase !== 'APRUEBO HITO H-03' ||
    receipt.previous?.lock_sha256 !== previousLockSha256 ||
    receipt.current?.lock_sha256 !== currentLockSha256 ||
    receipt.current?.package_sha256 !== currentPackageSha256 ||
    receipt.production_state !== 'BLOCKED_LICENSE' ||
