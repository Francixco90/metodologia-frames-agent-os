...
const req = (from: TaskWorkState, to: TaskWorkState, kind: 'replan' | 'gate-fail'): TaskTransitionRequest => ({
...
export const oracle: Oracle = {
  ...
  run: (): OracleOutcome => {
    ...
    checks.push({name: 'step1 BLOQUEADO->ESPECIFICADO (replan) accepted', passed: step1, detail: step1 ? 'accepted' : 'rejected'});
    const step2 = !attempt('BLOQUEADO', 'COMPILADO', 'gate-fail');
    checks.push({name: 'step2 BLOQUEADO->COMPILADO rejected', passed: step2, detail: step2 ? 'rejected' : 'accepted (illegal)'});
    const step3 = !attempt('BLOQUEADO', 'ESPECIFICADO', 'gate-fail');
    checks.push({name: 'step3 BLOQUEADO->ESPECIFICADO without replan evidence rejected', passed: step3, detail: step3 ? 'rejected (wrong evidence)' : 'accepted without replan'});
