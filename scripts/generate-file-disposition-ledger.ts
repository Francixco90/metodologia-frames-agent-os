...
type ArtifactClass = (typeof artifactClasses)[number];
type Disposition = (typeof dispositions)[number];
type OwnerId = (typeof ownerIds)[number];
...
const quarantinePrefix = 'skills/stitch-remotion-walkthrough/';

const generatorSourcePaths = new Set([
...
const supersessionByPath = new Map<string, string>();

const generatedTemplateBindings = [
  {
...
const ledgerProjectionPaths = new Set([
  'docs/program/file-disposition-ledger.yml',
  'docs/program/file-disposition-ledger.md',
]);

const isRuntimeGeneratedEvidence = (path: string): boolean =>
